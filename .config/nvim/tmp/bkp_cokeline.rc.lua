...
local error_clr = "#E43c50"
local warning_clr = get_hex("DiagnosticWarn", "fg")
...
local components = {
	...
	index = {
		...
		fg = function(buffer)
			return (buffer.diagnostics.errors ~= 0 and error_clr)
				or (buffer.diagnostics.warnings ~= 0 and warning_clr)
				or (buffer.is_modified and modified_clr)
				or text_clr
		end,
		truncation = { priority = 1 },
	},
